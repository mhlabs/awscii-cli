...
program
  ...
  .description(
    "Browses and visualises stack metrics for a CloudFormation stack's resources as ASCII diagrams"
  )
  .action(async (cmd) => {
    await handle(cmd);
  });
...
async function handle(cmd) {
      ...
      do {
        ...
        stackList.push(
          ...stacks.StackSummaries.map((p) => {
            return {
              name: demoUtil.obfuscateName(p.StackName, "service"),
              value: p.StackName,
            };
          })
        );
  ...
  const renderResult = await metricsUtil.render(
    ...
    mappings,
    [
      `https://${AWS.config.region}.console.aws.amazon.com/lambda/home?region=${
        AWS.config.region
      }#/applications/${demoUtil.reverseObfuscatedName(
        stackName
      )}?tab=monitoring`,
    ],
    `awscii stack --resource-type ${resourceType} --names ${resourceNames
      .map((p) => demoUtil.reverseObfuscatedName(p))
      .join(",")} --graph-types "#graphtypes#" --profile ${cmd.profile}`
  );
  if (cmd.save) {
    ...
    cmd.names = resourceNames
      .map((p) => demoUtil.reverseObfuscatedName(p))
      .join(",");
    await cacheUtil.saveCommand(cmd, "stack");
  }
}
